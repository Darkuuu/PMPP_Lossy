...
inline int getFileNameWithoutPath(char *path, char **filename)
{
	int string_length = (int)strlen(path);

	while ((path[string_length] != '/') && (path[string_length] != '\\'))
	{
		string_length--;
		if (string_length == 0)
			break;
